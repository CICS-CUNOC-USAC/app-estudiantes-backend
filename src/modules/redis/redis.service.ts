...
export class RedisService { 
    ...
    async saveDataset(saveDatasetDto: SaveDatasetDto): Promise<SaveDatasetDto> {
        saveDatasetDto.key = saveDatasetDto.key ?? Math.random().toString(36).substring(2, 7);
        saveDatasetDto.ttl = saveDatasetDto.ttl ?? this.configService.get<number>('redis.defaultTTL'); 
        saveDatasetDto.prefix = saveDatasetDto.prefix ?? this.configService.get<string>('redis.defaultPrefix'); 
    ...
    async getDataset(getDatasetDto: GetDatasetDto): Promise<string> {
        getDatasetDto.prefix = getDatasetDto.prefix ?? this.configService.get<string>('redis.defaultPrefix');
